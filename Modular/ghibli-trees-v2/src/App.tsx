...
const assets = [
  { id: 'oak', name: 'Ghibli Oak', generate: generateOak },
  { id: 'pine', name: 'Windswept Pine', generate: generatePine },
  { id: 'custom', name: 'Custom Tree (Interactive)', generate: () => null }, // Handled dynamically in useEffect
  { id: 'spire', name: 'Wizard Spire', generate: generateSpire },
];

export default function App() {
  const canvasContainer = useRef<HTMLDivElement>(null);
  const [activeId, setActiveId] = useState('custom'); // Show Custom Tree by default for instant interactivity!
  const [autoRotate, setAutoRotate] = useState(false); // Off by default to allow easy customization view
  ...
  const handleParamChange = (key: keyof TreeParams, value: number | boolean) => {
    setParams(prev => ({ ...prev, [key]: value }));
    setActiveId('custom'); // Automatically toggle active asset to custom tree!
  };

  const randomizeSeed = () => {
    const profiles = ['oak', 'birch', 'bonsai'];
    ...
    if (profile === 'oak') {
      newHeight = 7.0 + Math.random() * 4.0; // 7 to 11
      newRadius = newHeight * (0.08 + Math.random() * 0.04);
      newBranchLength = newHeight * (0.3 + Math.random() * 0.15); // max 45%
      newBranchAngle = 35 + Math.random() * 20; // 35 to 55
      ...
      newHeight = 9.0 + Math.random() * 4.0; // 9 to 13
      newRadius = newHeight * (0.05 + Math.random() * 0.03);
      newBranchLength = newHeight * (0.2 + Math.random() * 0.1);
      newBranchAngle = 20 + Math.random() * 15; // 20 to 35
      newPuffballSize = newBranchLength * (0.4 + Math.random() * 0.3);
      newFoliageFlattening = 0.9 + Math.random() * 0.3;
      newGnarliness = 0.1 + Math.random() * 0.2;
      newBranchSplits = 2;
    } else { // bonsai
      newHeight = 3.0 + Math.random() * 3.0; // 3 to 6
      newRadius = newHeight * (0.1 + Math.random() * 0.05);
      newBranchLength = newHeight * (0.35 + Math.random() * 0.1);
      newBranchAngle = 40 + Math.random() * 15; // 40 to 55
    ...
    setParams(prev => ({
      ...
      puffballCount: 5 + Math.floor(Math.random() * 8), // Per-tip count
